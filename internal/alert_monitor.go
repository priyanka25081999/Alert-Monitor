...
// The purpose of this function is to create and return a new AlertMonitor instance with its internal state (eventLogs) properly initialized.
// This initializes the eventLogs field of the AlertMonitor struct. eventLogs is a map where the keys are strings, and the values are slices of time.Time objects.
...
func (am *AlertMonitor) checkThreshold(config types.AlertConfig, key string) bool {
	...
	case types.TumblingWindowConfig:
		// TODO: check example by taking actual timestamp/time
